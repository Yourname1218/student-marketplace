...
console.log('🚀 開始建立範例資料...\n')

// 清空現有資料
db.exec(`DELETE FROM favorites; DELETE FROM comments; DELETE FROM products; DELETE FROM users;`)
...
for (const user of users) {
  const result = db.prepare('INSERT INTO users (username, email, password, school, phone) VALUES (?, ?, ?, ?, ?)').run(user.username, user.email, hashedPassword, user.school, user.phone)
...
console.log(`✅ 完成！建立了 ${users.length} 位使用者和 ${productCount} 個商品\n`)
console.log('🔑 測試帳號: xiaoming@student.edu.tw / 123456\n')
